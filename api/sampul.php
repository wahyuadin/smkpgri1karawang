...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['id']) && isset($_FILES["sampul"]) && isset($_POST['token'])) {
        $token  = htmlspecialchars($_POST['token']);
        $id     = htmlspecialchars($_POST['id']);
        $secret_key = bearer();
        try {
            $decoded = JWT::decode($token, new Key($secret_key, 'HS256'));
            $uploaded_file = $_FILES["sampul"];
            $image_path    = Sampul($uploaded_file, $id);
            if ($image_path) {
                $response = array(
                    'status'    => 'success',
    ...
    if (isset($_GET['id']) && isset($_GET['token'])) {
        ...
        try {
            ...
            if (GetData($id)->num_rows > 0) {
                $response = [
                    'status'    => 'success',
                    'data'      => base_url().GetData($id)->fetch_object()->sampul
                ];
            }
